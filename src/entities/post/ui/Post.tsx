...
import {CustomSwiper, IconBlock} from "@rocketweb-studio/ulens-ui-kit";
...
export const Post = ({postItem}: Props) => {

  return (
    <div className='flex flex-col gap-[12px] w-[240px]'>
     <CustomSwiper                   slides={postItem.images.medium.map((image, index) => ({
       id: index,
       content: (
           <img key={postItem.id} className='w-[240px] h-[240px] object-cover' src={`${import.meta.env.VITE_MEDIA_URL}${image.url}`} alt="post photo"/>
       ),
     }))}/>
